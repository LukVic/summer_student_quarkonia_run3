...
ht_de = template_file.Get("ht").Rebin(1)
ht_de.Scale(1. / ht_de.Integral())
htnon_de = template_file.Get("htnon").Rebin(1)
htnon_de.Scale(1. / htnon_de.Integral())
hm_de = template_file.Get("hm").Rebin(1)
hm_de.Scale(1. / hm_de.Integral())
hmnon_de = template_file.Get("hmnon").Rebin(1)
hmnon_de.Scale(1. / hmnon_de.Integral())


data_file = ROOT.TFile(PATH_DATA + "data_tune_45_4-6_sign_plus.root")

ht_data_bg =  data_file.Get("ht_cut").Rebin(1)
ht_data_bg.Scale(1. / ht_data_bg.Integral())

hm_data = data_file.Get("hm").Rebin(1)
ht_data = data_file.Get("ht").Rebin(1)
...
# n = ROOT.RooRealVar("n", "n", 5, 0, 150)
# cb_pdf = ROOT.RooCBShape("cb_pdf", "Crystal Ball PDF", mass, mean, sigma, alpha, n)

# cheb_coeffs = [ROOT.RooRealVar(f"cheb_coeff_{i}", f"Coeff_{i}", 0.01, -3, 3) for i in range(6)]
# cheb_poly = ROOT.RooChebychev("cheb_poly", "Chebyshev Polynomial", mass, ROOT.RooArgList(*cheb_coeffs))
